...
NSComparisonResult compareViewDepthForAd(UIView *view1, UIView *view2, ECAdCarousel *self);

@end


@implementation ECAdCarousel
...
- (void)setDelegate:(id<iCarouselDelegate>)delegate
{
    if (_delegate != delegate)
    {
        ...
        {
            ECLog(@"Warning: The carousel:itemAlphaForOffset: delegate method is deprecated. Use carousel:valueForOption:withDefault: with iCarouselOptionFadeMin, iCarouselOptionFadeMax and iCarouselOptionFadeRange instead");
        }
        if ([_delegate respondsToSelector:@selector(carouselShouldWrap:)])
        {
            ECLog(@"Warning: The carouselShouldWrap: delegate method is deprecated. Use carousel:valueForOption:withDefault: with iCarouselOptionWrap instead");
        }
        if ([_delegate respondsToSelector:@selector(carouselOffsetMultiplier:)])
        {
            ECLog(@"Warning: The carouselOffsetMultiplier: delegate method is deprecated. Use carousel:valueForOption:withDefault: with iCarouselOptionOffsetMultiplier instead");
        }
        if ([_delegate respondsToSelector:@selector(numberOfVisibleItemsInCarousel:)])
        {
            ECLog(@"Warning: The numberOfVisibleItemsInCarousel: dataSource method is deprecated. This value is now set automatically, but if you need to override it, use carousel:valueForOption:withDefault: with iCarouselOptionVisibleItems instead");
        }
        if ([_delegate respondsToSelector:@selector(carouselCurrentItemIndexUpdated:)])
        {
            ECLog(@"Warning: The carouselCurrentItemIndexUpdated: delegate method is deprecated. Use carouselCurrentItemIndexDidChange: instead");
        }
...
- (void)depthSortViews
{
    for (UIView *view in [[_itemViews allValues] sortedArrayUsingFunction:(NSInteger (*)(id, id, void *))compareViewDepthForAd context:(__bridge void *)self])
    {
...
- (void)mouseUp:(NSEvent *)theEvent
{
    if (!_didDrag)
    {
        ...
        for (UIView *view in [[[_itemViews allValues] sortedArrayUsingFunction:(NSInteger (*)(id, id, void *))compareViewDepthForAd context:(__bridge void *)self] reverseObjectEnumerator])
        {
